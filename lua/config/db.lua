...
local function ssh_tunnel_and_connect()
	...
	local ssh_command =
		string.format("ssh -i %s -L %s:%s:%s %s@%s -N -v", key_path, local_port, rds_host, rds_port, user, domain)
	...
	local connection_url = string.format("postgres://%s:%s@127.0.0.1:%s/%s", db_user, db_password, local_port, db_name)
